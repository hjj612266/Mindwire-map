...
const CardSystem = {
  ...
  renderCard(card, animate) {
    ...
    el.className = 'card' + (animate ? ' entering' : '') + (card.collapsed ? ' collapsed' : '') + (card.id === this.selectedId ? ' selected' : '');
    ...
    const collapseIcon = card.collapsed ? 'fa-plus' : 'fa-minus';
    ...
    el.innerHTML = `
      <div class="card-header">
        ...
        <div class="card-actions">
          ...
          <button class="card-action-btn" data-action="collapse" title="折叠"><i class="fa-solid ${collapseIcon}"></i></button>
          <button class="card-action-btn danger" data-action="delete" title="删除"><i class="fa-solid fa-xmark"></i></button>
        </div>
      </div>
  ...
  bindCardEvents(el, card) {
    ...
    el.querySelectorAll('.card-action-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        else if (action === 'collapse') this.toggleCollapse(card.id);
  ...
  toggleCollapse(id) {
    ...
    if (el) {
      ...
      const icon = el.querySelector('[data-action="collapse"] i');
  ...
  onAnchorDragStart(e, cardId, dir) {
    ...
    const updateLine = (me) => {
      ...
      if (targetAnchor && targetAnchor.classList.contains('anchor') && !targetAnchor.closest(`.card[data-id="${cardId}"]`)) {
